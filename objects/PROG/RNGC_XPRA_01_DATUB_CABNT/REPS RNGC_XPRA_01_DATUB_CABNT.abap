*&---------------------------------------------------------------------*
*& Report RNGC_XPRA_01_DATUB_CABNT
*&---------------------------------------------------------------------*
...
REPORT rngc_xpra_01_datub_cabnt.
  ...
  DATA:
    ...
    lo_datub_cabnt   TYPE REF TO cl_ngc_core_xpra_datub_cabnt.
...
START-OF-SELECTION.
  ...
  lv_exit = cl_ngc_core_xpra_datub_cabnt=>before_conversion( iv_client = client  iv_rework = rework ).
  ...
  IF client EQ gc_allclients.
    LOOP AT cl_plm_xpra_utilities=>get_clients( ) ASSIGNING FIELD-SYMBOL(<ls_mandt>).
      lo_datub_cabnt = NEW #( iv_appl_name = gc_appl_name  iv_client = <ls_mandt>-mandt  iv_rework = rework ).
      lo_datub_cabnt->convert_client( ).
    ENDLOOP.
  ELSE.
    lo_datub_cabnt = NEW #( iv_appl_name = gc_appl_name  iv_client = client  iv_rework = rework ).
    lo_datub_cabnt->convert_client( ).
  ENDIF.

  " finish conversion
  cl_ngc_core_xpra_datub_cabnt=>after_conversion( ).