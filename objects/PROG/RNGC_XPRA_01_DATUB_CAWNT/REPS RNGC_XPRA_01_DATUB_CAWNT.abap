*&---------------------------------------------------------------------*
*& Report RNGC_XPRA_01_DATUB_CAWNT
*&---------------------------------------------------------------------*
...
REPORT rngc_xpra_01_datub_cawnt.
  ...
  DATA:
    ...
    lo_datub_cawnt   TYPE REF TO cl_ngc_core_xpra_datub_cawnt.
...
START-OF-SELECTION.
  ...
  lv_exit = cl_ngc_core_xpra_datub_cawnt=>before_conversion( iv_client = client  iv_rework = rework ).
  ...
  IF client EQ gc_allclients.
    LOOP AT cl_plm_xpra_utilities=>get_clients( ) ASSIGNING FIELD-SYMBOL(<ls_mandt>).
      lo_datub_cawnt = NEW #( iv_appl_name = gc_appl_name  iv_client = <ls_mandt>-mandt  iv_rework = rework ).
      lo_datub_cawnt->convert_client( ).
    ENDLOOP.
  ELSE.
    lo_datub_cawnt = NEW #( iv_appl_name = gc_appl_name  iv_client = client  iv_rework = rework ).
    lo_datub_cawnt->convert_client( ).
  ENDIF.

  " finish conversion
  cl_ngc_core_xpra_datub_cawnt=>after_conversion( ).