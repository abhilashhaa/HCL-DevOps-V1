*&---------------------------------------------------------------------*
*& Report RNGC_XPRA_01_DATUB_AUSP
*&---------------------------------------------------------------------*
...
REPORT rngc_xpra_01_datub_ausp.
  ...
  DATA:
    ...
    lo_datub_ausp    TYPE REF TO cl_ngc_core_xpra_datub_ausp.
...
START-OF-SELECTION.
  ...
  lv_exit = cl_ngc_core_xpra_datub_ausp=>before_conversion( iv_client = client  iv_rework = rework ).
  ...
  IF client EQ gc_allclients.
    LOOP AT cl_plm_xpra_utilities=>get_clients( ) ASSIGNING FIELD-SYMBOL(<ls_mandt>).
      lo_datub_ausp = NEW #( iv_appl_name = gc_appl_name  iv_client = <ls_mandt>-mandt  iv_rework = rework ).
      lo_datub_ausp->convert_client( ).
    ENDLOOP.
  ELSE.
    lo_datub_ausp = NEW #( iv_appl_name = gc_appl_name  iv_client = client  iv_rework = rework ).
    lo_datub_ausp->convert_client( ).
  ENDIF.

  " finish conversion
  cl_ngc_core_xpra_datub_ausp=>after_conversion( ).