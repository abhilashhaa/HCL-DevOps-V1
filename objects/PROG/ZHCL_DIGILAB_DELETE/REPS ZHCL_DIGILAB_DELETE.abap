...
*&---------------------------------------------------------------------*
FORM GET_DATA .
  LV_DATE = SY-DATUM - 45.                                          "Calculate Date

**********************Fetch Logon Data from 'USR02' table
  SELECT BNAME GLTGV GLTGB USTYP UFLAG TRDAT
  FROM USR02
  INTO CORRESPONDING FIELDS OF TABLE IT_USER
  WHERE TRDAT LE LV_DATE
  AND USTYP EQ 'A'                                                    "Dialog User
  AND UFLAG BETWEEN 60 AND 192.                                       " Loked user by admmin
...
FORM PREPARE_DATA .
  ...
  LOOP AT IT_USER INTO WA_USER.
    ...
    MOVE WA_USER-UFLAG TO WA_FINAL-UFLAG.
    ...
    READ TABLE  IT_DD07V_TAB_A INTO WA_DD07V_TAB_A WITH KEY DOMVALUE_L = WA_USER-USTYP.
    IF SY-SUBRC = 0.
...
* Log dleted user in a separate Z table zhcl_deldata.
    PERFORM F_SAVE_DEL_LOG USING WA_FINAL-BNAME.
...
*      MOVE 'Delete TO wa_log-trdat.
      CONCATENATE 'Delete User'  WA_FINAL-BNAME INTO WA_LOG-STATUS SEPARATED BY SPACE.
...
*&      --> WA_FINAL_BNAME
*&---------------------------------------------------------------------*
FORM F_SAVE_DEL_LOG  USING F_USERNAME TYPE XUBNAME."   P_WA_FINAL_BNAME.

* data Declarations:
  DATA : LS_DELDATA   TYPE ZHCL_DELDATA,
         LV_TIMESTAMP TYPE TIMESTAMP.
  GET TIME STAMP FIELD LV_TIMESTAMP.

  LS_DELDATA-MANDT = SY-MANDT.
  LS_DELDATA-DEL_TIME = LV_TIMESTAMP.
  LS_DELDATA-DEL_USER = SY-UNAME.
  LS_DELDATA-DEL_OBJ = F_USERNAME.

  MODIFY ZHCL_DELDATA FROM LS_DELDATA.
...
FORM SAVE_DB  TABLES   P_IT_LOG .
  ...
  SORT P_IT_LOG1 BY BNAME.
  DELETE ADJACENT DUPLICATES FROM P_IT_LOG1 COMPARING BNAME.

  IF P_IT_LOG1[] IS NOT INITIAL.
    MODIFY ZHCL_DELUSR FROM TABLE P_IT_LOG1.
  ENDIF.
