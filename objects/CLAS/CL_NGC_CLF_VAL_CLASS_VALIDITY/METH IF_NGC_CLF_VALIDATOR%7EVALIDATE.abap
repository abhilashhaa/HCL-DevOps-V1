...
*--------------------------------------------------------------------*
* Checking the validity of the class to be assigned.
...
*--------------------------------------------------------------------*
  ...
  ASSIGNING FIELD-SYMBOL(<ls_assigned_classes_upd>)
    ...
    IF ls_classification_key-key_date < ls_class_header-validitystartdate
    OR ls_classification_key-key_date > ls_class_header-validityenddate.
      ...
      " Error handling: Class type &1: class &2 is not valid on &3
      MESSAGE e004(ngc_api_base) WITH ls_class_header-classtype ls_class_header-class ls_classification_key-key_date INTO DATA(lv_msg) ##NEEDED.
