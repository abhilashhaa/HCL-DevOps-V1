...
CLASS lth_ngc_clf_val_class_validity DEFINITION FINAL.
  PUBLIC SECTION.
...
CLASS lth_ngc_clf_val_class_validity IMPLEMENTATION.
ENDCLASS.
...
CLASS ltc_ngc_clf_val_class_validity DEFINITION DEFERRED.
CLASS cl_ngc_clf_val_class_validity DEFINITION LOCAL FRIENDS ltc_ngc_clf_val_class_validity.

CLASS ltc_ngc_clf_val_class_validity DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA:
      mo_cut TYPE REF TO cl_ngc_clf_val_class_validity.  "class under test
    ...
    METHODS: clf_validity_ok FOR TESTING.
    METHODS: clf_validity_fail FOR TESTING.
...
CLASS ltc_ngc_clf_val_class_validity IMPLEMENTATION.
  ...
  METHOD no_clf.
    ...
    DATA(lo_clf_api_result) = mo_cut->if_ngc_clf_validator~validate( iv_classtype      = lth_ngc_clf_val_class_validity=>gc_classtype
                                                                     io_data_provider  = NEW ltd_ngc_clf_validation_dp( )
                                                                     io_classification = get_classification_stub( abap_false ) ).
  ...
  METHOD wrong_classtype.
    ...
    DATA(lo_clf_api_result) = mo_cut->if_ngc_clf_validator~validate( iv_classtype      = lth_ngc_clf_val_class_validity=>gc_another_classtype
                                                                     io_data_provider  = NEW ltd_ngc_clf_validation_dp( )
                                                                     io_classification = get_classification_stub( ) ).
  ...
  METHOD assert_error_message.
    MESSAGE e004(ngc_api_base) WITH lth_ngc_clf_val_class_validity=>gc_classtype lth_ngc_clf_val_class_validity=>gc_class lth_ngc_clf_val_class_validity=>gc_key_date INTO DATA(lv_msg) ##NEEDED.
    READ TABLE it_messages TRANSPORTING NO FIELDS WITH KEY object_key       = lth_ngc_clf_val_class_validity=>gc_object_key_one
                                                           technical_object = lth_ngc_clf_val_class_validity=>gc_technical_object
                                                           key_date         = lth_ngc_clf_val_class_validity=>gc_key_date
                                                           change_number    = space
                                                           msgid            = sy-msgid
                                                           msgty            = sy-msgty
                                                           msgno            = sy-msgno
  ...
  METHOD get_classification_stub.
    ...
    cl_abap_testdouble=>configure_call( ro_classification )->ignore_all_parameters( )->returning(
      VALUE ngcs_classification_key(
        object_key       = lth_ngc_clf_val_class_validity=>gc_object_key_one
        technical_object = lth_ngc_clf_val_class_validity=>gc_technical_object
        key_date         = lth_ngc_clf_val_class_validity=>gc_key_date
        change_number    = space ) ).
    ro_classification->get_classification_key( ).

    IF iv_include_clf = abap_true.
      cl_abap_testdouble=>configure_call( ro_classification )->ignore_all_parameters(
      )->set_parameter(
        name  = 'et_classification_data_upd'
        value = VALUE ngct_classification_data_upd( ( classinternalid = lth_ngc_clf_val_class_validity=>gc_classinternalid ) )
      )->set_parameter(
        name  = 'et_assigned_class_upd'
        value = VALUE ngct_class_object_upd( (
          classinternalid = lth_ngc_clf_val_class_validity=>gc_classinternalid
          key_date        = lth_ngc_clf_val_class_validity=>gc_key_date
          class_object    = NEW ltd_ngc_cls( VALUE #( classinternalid   = lth_ngc_clf_val_class_validity=>gc_classinternalid
                                                      classtype         = lth_ngc_clf_val_class_validity=>gc_classtype
                                                      class             = lth_ngc_clf_val_class_validity=>gc_class
                                                      validitystartdate = lth_ngc_clf_val_class_validity=>gc_key_date + 10
                                                      validityenddate   = lth_ngc_clf_val_class_validity=>gc_key_date + 20 ) )
