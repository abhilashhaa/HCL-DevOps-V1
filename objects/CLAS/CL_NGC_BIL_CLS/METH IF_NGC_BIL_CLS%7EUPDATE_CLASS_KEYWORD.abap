  METHOD if_ngc_bil_cls~update_class_keyword.

    FIELD-SYMBOLS:
      <ls_class_change>  TYPE lty_s_class_change,
      <ls_class_keyword> TYPE lty_s_class_keyword.

    CLEAR: et_failed, et_reported.

    LOOP AT it_update ASSIGNING FIELD-SYMBOL(<ls_update>).
      UNASSIGN: <ls_class_change>, <ls_class_keyword>.

      me->load_class_data( <ls_update>-classinternalid ).
      ASSIGN mt_class_change[ classinternalid = <ls_update>-classinternalid ] TO <ls_class_change>.

      IF <ls_class_change> IS NOT ASSIGNED.
        " Class does not exist
        MESSAGE e002(ngc_rap) INTO DATA(lv_dummy) ##NEEDED.
        me->add_classkeyword_message(
          EXPORTING
            iv_classinternalid            = <ls_update>-classinternalid
            iv_language                   = <ls_update>-language
            iv_classkeywordpositionnumber = <ls_update>-classkeywordpositionnumber
            iv_cid                        = <ls_update>-%cid_ref
          CHANGING
            ct_reported                   = et_reported
            ct_failed                     = et_failed ).

        CONTINUE.
      ENDIF.

      DATA(ls_class_change) = <ls_class_change>.

      ASSIGN ls_class_change-t_classkeyword_new[ langu = <ls_update>-language classkeywordpositionnumber = <ls_update>-classkeywordpositionnumber ] TO <ls_class_keyword>.

      IF <ls_class_keyword> IS NOT ASSIGNED.
        " No keywordription defined in language &1
        MESSAGE e011(ngc_rap) WITH <ls_update>-language INTO lv_dummy ##NEEDED.
        me->add_classkeyword_message(
          EXPORTING
            iv_classinternalid            = <ls_update>-classinternalid
            iv_language                   = <ls_update>-language
            iv_classkeywordpositionnumber = <ls_update>-classkeywordpositionnumber
            iv_cid                        = <ls_update>-%cid_ref
          CHANGING
            ct_reported                   = et_reported
            ct_failed                     = et_failed ).

        DELETE mt_class_change
          WHERE
            classinternalid = <ls_update>-classinternalid.

        CONTINUE.
      ENDIF.

      DATA(ls_classkeyword_merged) = me->map_classkeyword_vdm_api(
        EXPORTING
          is_classkeyword_vdm_upd = <ls_update>
          is_classkeyword_new_api = <ls_class_keyword> ).

      <ls_class_keyword> = ls_classkeyword_merged.

      DATA(lt_return) = me->modify_class_data(
        is_class_data   = ls_class_change
        iv_clear_buffer = abap_true ).

      LOOP AT lt_return ASSIGNING FIELD-SYMBOL(<ls_return>)
        WHERE
          type CA gc_error_types.
        me->add_classkeyword_message(
          EXPORTING
            iv_classinternalid            = <ls_update>-classinternalid
            iv_language                   = <ls_update>-language
            iv_classkeywordpositionnumber = <ls_update>-classkeywordpositionnumber
            iv_cid                        = <ls_update>-%cid_ref
            is_bapiret                    = <ls_return>
          CHANGING
            ct_reported                   = et_reported
            ct_failed                     = et_failed ).
      ENDLOOP.

      IF sy-subrc <> 0.
        <ls_class_change> = ls_class_change.

        APPEND gc_operation_type-update_keyword TO <ls_class_change>-t_operation_log.
      ENDIF.
    ENDLOOP.

  ENDMETHOD.