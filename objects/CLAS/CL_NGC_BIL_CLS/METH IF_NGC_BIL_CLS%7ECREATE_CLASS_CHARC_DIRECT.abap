  METHOD if_ngc_bil_cls~create_class_charc_direct.

    DATA:
      lt_create_from_class TYPE if_ngc_bil_cls_c=>lty_clfnclasscharctp-t_create.
    ...
    LOOP AT it_create ASSIGNING FIELD-SYMBOL(<ls_create>)
      ...
      LOOP AT GROUP ls_create_group ASSIGNING FIELD-SYMBOL(<ls_create_group>).
        APPEND INITIAL LINE TO <ls_create_from_class>-%target ASSIGNING FIELD-SYMBOL(<ls_target>).
        <ls_target> = CORRESPONDING #( <ls_create_group> ).
      ENDLOOP.
    ENDLOOP.

    me->if_ngc_bil_cls~create_class_charc(
      EXPORTING
