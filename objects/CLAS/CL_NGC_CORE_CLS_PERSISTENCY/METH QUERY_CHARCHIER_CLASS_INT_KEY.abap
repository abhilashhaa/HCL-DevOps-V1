METHOD query_charchier_class_int_key.

* Assumption: IT_CLASS_RANGE_BY_CLINT contains list of class internal ids, with only
...
*      - Characteristic:          name of the inherited char.
  ...
  DATA:
    ...
    lt_ancestor_range              TYPE tt_classinternalid_range,
    lv_lines_already_read          TYPE int4 VALUE 0,
    lt_class_range_to_be_read      TYPE tt_classinternalid_range.

  lt_class_range_by_clint = it_class_range_by_clint.

  " if MT_CLFNCLASSHIERCHARC already contain this class,
  " we will not go further with the selections with this class
  LOOP AT lt_class_range_by_clint ASSIGNING FIELD-SYMBOL(<ls_class_int_key_with_range>).
    LOOP AT <ls_class_int_key_with_range>-classinternalid ASSIGNING FIELD-SYMBOL(<ls_range_classinternalid>)
      ...
      READ TABLE mt_clfnclasshiercharc TRANSPORTING NO FIELDS
        WITH KEY key_date        = <ls_class_int_key_with_range>-key_date
                 classinternalid = <ls_range_classinternalid>-low.
      IF sy-subrc = 0.
        DELETE <ls_class_int_key_with_range>-classinternalid.
      ENDIF.
    ENDLOOP.
    IF <ls_class_int_key_with_range>-classinternalid IS INITIAL.
      DELETE lt_class_range_by_clint.
    ENDIF.
  ENDLOOP.

  LOOP AT lt_class_range_by_clint ASSIGNING <ls_class_int_key_with_range>.
    CLEAR: lt_clfnclasshiercharcforkeydat, lv_lines_already_read.

    WHILE lv_lines_already_read < lines( <ls_class_int_key_with_range>-classinternalid ).
      CLEAR: lt_class_range_to_be_read.
      APPEND LINES OF <ls_class_int_key_with_range>-classinternalid
      ...
      TEST-SEAM sel_cds_classcharc_int_key.
        SELECT
          @<ls_class_int_key_with_range>-key_date AS key_date,
          ...
          FROM i_clfnclasssuperiorforkeydate( p_keydate = @<ls_class_int_key_with_range>-key_date )  AS classsuperior
             LEFT OUTER JOIN i_clfnclasscharcbasic   AS classcharcbasic
               ...
               AND ( ( classcharcbasic~validitystartdate <= @<ls_class_int_key_with_range>-key_date
                   AND classcharcbasic~validityenddate   >= @<ls_class_int_key_with_range>-key_date )
             ...
             LEFT OUTER JOIN i_clfncharcbasic        AS characteristic
               ...
               AND characteristic~validitystartdate  <= @<ls_class_int_key_with_range>-key_date
               AND characteristic~validityenddate    >= @<ls_class_int_key_with_range>-key_date
               AND characteristic~isdeleted          =  ''
           WHERE classsuperior~classinternalid IN @lt_class_range_to_be_read
...
* In addition to this, MT_CLFNCLASSHIERCHARCFORKEYDAT will contain at least one additional
...
*--------------------------------------------------------------------*
    ...
    IF lt_ancestor_range IS NOT INITIAL.
      ...
      WHILE lv_lines_already_read < lines( lt_ancestor_range ).
        ...
        TEST-SEAM sel_cds_anc_classcharc_int_key.
          SELECT
            @<ls_class_int_key_with_range>-key_date AS key_date,
            classsuperior~classinternalid,
            classsuperior~ancestorclassinternalid,
...
*            classcharcbasic~clfnorganizationalarea
    ...
    SORT mt_clfnclasshiercharcforkeydat ASCENDING BY classinternalid ancestorclassinternalid charcinternalid key_date.
    DELETE ADJACENT DUPLICATES FROM mt_clfnclasshiercharcforkeydat COMPARING classinternalid ancestorclassinternalid charcinternalid key_date.
