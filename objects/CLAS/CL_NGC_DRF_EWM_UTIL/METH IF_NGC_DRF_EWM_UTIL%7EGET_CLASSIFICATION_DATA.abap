METHOD if_ngc_drf_ewm_util~get_classification_data.
  ...
  IF lines( lt_singleobj_bo_keys ) > 0.
    ...
    SELECT ausp~*
      FROM (
        ausp
        INNER JOIN kssk ON ausp~objek = kssk~objek
        INNER JOIN ksml ON kssk~clint = ksml~clint AND ausp~atinn = ksml~imerk
      )
      FOR ALL ENTRIES IN @lt_singleobj_bo_keys
      WHERE ausp~klart = @lt_singleobj_bo_keys-klart AND ausp~objek = @lt_singleobj_bo_keys-objkey AND ksml~ewm_rel = 'X'
  ...
  IF lines( lt_multobj_bo_keys ) > 0.
    ...
    SELECT ausp~*
      FROM (
        ausp
        INNER JOIN inob ON ausp~klart = inob~klart AND ausp~objek = inob~cuobj
        INNER JOIN kssk ON ausp~objek = kssk~objek
        INNER JOIN ksml ON kssk~clint = ksml~clint AND ausp~atinn = ksml~imerk
      )
      FOR ALL ENTRIES IN @lt_multobj_bo_keys
      WHERE inob~klart = @lt_multobj_bo_keys-klart AND inob~obtab = @lt_multobj_bo_keys-object_table AND inob~cuobj = @lt_multobj_bo_keys-objkey AND ksml~ewm_rel = 'X'
