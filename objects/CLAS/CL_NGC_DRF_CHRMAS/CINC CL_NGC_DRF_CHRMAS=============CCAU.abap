...
CLASS lth_drf_access_cust_data DEFINITION FINAL.
  PUBLIC SECTION.
    CLASS-DATA: gt_chr_key TYPE ngct_drf_chrmas_object_key.
    CLASS-METHODS: class_constructor.
ENDCLASS.

CLASS lth_drf_access_cust_data IMPLEMENTATION.
  METHOD class_constructor.
    gt_chr_key = VALUE #( ( atnam = 'CHR_1' )
                          ( atnam = 'CHR_2' )
                          ( atnam = 'CHR_3' ) ).
  ENDMETHOD.
ENDCLASS.
...
CLASS ltc_ngc_chrmas_drf DEFINITION DEFERRED.
CLASS cl_ngc_drf_chrmas DEFINITION LOCAL FRIENDS ltc_ngc_chrmas_drf.

CLASS ltc_ngc_chrmas_drf DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA:
      f_cut TYPE REF TO cl_ngc_drf_chrmas.  "class under test
...
CLASS ltc_ngc_chrmas_drf IMPLEMENTATION.
...
*    ).
  ...
  METHOD initialize.
    ...
    cl_ngc_drf_chrmas=>if_drf_outbound~initialize(
      EXPORTING
    ...
    cl_abap_unit_assert=>assert_equals(
      act = cl_ngc_drf_chrmas=>go_drf_chrmas->ms_runtime_param-appl
      exp = 'TESTAPPL'
      msg = 'initialize: CL_CACL_COM_CHRMAS_DRF=>GO_DRF_CHRMAS->MS_RUNTIME_PARAM-APPL is not valid'
    ).

    cl_abap_unit_assert=>assert_equals(
      act = es_runtime_param_out_impl-table_type_name
      exp = if_ngc_drf_c=>gc_chrmas_drf_table_type
