...
*--------------------------------------------------------------------*
    ...
    DATA(lt_test_data) = th_ngc_bil_clf_data=>gt_objectcharcval_delete_singl.
    ...
    setup_get_characteristics(
      ...
      it_characteristic             = VALUE ngct_clf_characteristic_object( ( mt_ngc_characteristic[ 1 ] ) )
      io_ngc_clf_api_result         = lo_clf_api_result ).

    setup_get_assigned_values(
      iv_number_of_expected_calling = 1
      io_clf_api_result             = lo_clf_api_result
      it_valuation_data             = th_ngc_bil_clf_data=>gt_objectcharcval_val_data_sng ).
    ...
    setup_delete_num_value(
      iv_number_of_expected_calling = 0
      io_clf_api_result             = lo_clf_api_result
      it_success_value              = VALUE #( )
    ).
...
*--------------------------------------------------------------------*
    ...
    cl_abap_testdouble=>verify_expectations( double = mt_ngc_characteristic[ 1 ]-characteristic_object ).
