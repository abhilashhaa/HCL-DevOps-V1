...
*--------------------------------------------------------------------*
    DATA:
      ...
      lt_characteristic     TYPE ngct_clf_characteristic_object.

    DATA(lt_input) = th_ngc_bil_clf_data=>gt_objectcharcval_read_by_chr.

    LOOP AT lt_input ASSIGNING FIELD-SYMBOL(<ls_input>).
      APPEND INITIAL LINE TO lt_characteristic ASSIGNING FIELD-SYMBOL(<ls_characteristic>).
      <ls_characteristic>-charcinternalid       = <ls_input>-charcinternalid.
      <ls_characteristic>-characteristic_object = mo_ngc_characteristic.
    ENDLOOP.
...
*--------------------------------------------------------------------*
    ...
    cl_abap_unit_assert=>assert_true(
      EXPORTING
        act = COND abap_bool( WHEN lines( lt_result ) = 4 THEN abap_true ELSE abap_false ) ).
