...
*--------------------------------------------------------------------*
    ...
    DATA(lt_input) = VALUE if_ngc_bil_clf=>ts_objcharc-read_by-_objectcharcvalue-t_input(
      ( th_ngc_bil_clf_data=>gt_objectcharcval_read_by_chr[ 1 ] )
    ).
...
*--------------------------------------------------------------------*
    ...
    cl_abap_unit_assert=>assert_true(
      EXPORTING
        act = COND abap_bool( WHEN lines( lt_result ) = 1 THEN abap_true ELSE abap_false )
    ).
