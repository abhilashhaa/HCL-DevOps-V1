  METHOD create_op_class_assign_failure.
...
*--------------------------------------------------------------------*
    ...
    DATA(lo_clf_api_result) = setup_api_result(
      it_message = VALUE #(
        ( object_key       = th_ngc_bil_clf_data=>gt_objectclass_create_single[ 1 ]-clfnobjectid
          technical_object = th_ngc_bil_clf_data=>gt_objectclass_create_single[ 1 ]-clfnobjecttable
          msgty            = 'E' ) ) ).
