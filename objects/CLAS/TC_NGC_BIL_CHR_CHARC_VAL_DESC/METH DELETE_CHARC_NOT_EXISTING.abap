  METHOD delete_charc_not_existing.

*--------------------------------------------------------------------*
* Arrangements
*--------------------------------------------------------------------*

    DATA(lt_delete) = VALUE if_ngc_bil_chr_c=>lty_clfncharcvaldesctp-t_delete(
      ( %cid_ref                 = 'CID_01'
        charcinternalid          = th_ngc_bil_chr_data=>cv_charc_num_id
        charcvaluepositionnumber = 2
        language                 = 'EN' ) ).

    DATA(lt_reported_exp) = VALUE if_ngc_bil_chr_c=>lty_clfncharcvaldesctp-t_reported(
      ( %cid = 'CID_01' language = 'EN' charcvaluepositionnumber = 2 charcinternalid = th_ngc_bil_chr_data=>cv_charc_num_id ) ).

    DATA(lt_failed_exp) = VALUE if_ngc_bil_chr_c=>lty_clfncharcvaldesctp-t_failed(
      ( %cid = 'CID_01' language = 'EN' charcvaluepositionnumber = 2 charcinternalid = th_ngc_bil_chr_data=>cv_charc_num_id ) ).

*--------------------------------------------------------------------*
* Actions
*--------------------------------------------------------------------*

    mo_cut->if_ngc_bil_chr~delete_charc_val_desc(
      EXPORTING
        it_delete   = lt_delete
      IMPORTING
        et_failed   = DATA(lt_failed)
        et_reported = DATA(lt_reported) ).

*--------------------------------------------------------------------*
* Assertions
*--------------------------------------------------------------------*

    cl_abap_unit_assert=>assert_equals(
      act = lt_failed
      exp = lt_failed_exp
      msg = 'Expexted failed should be returned' ).

    cl_abap_unit_assert=>assert_equals(
      act = lt_reported
      exp = lt_reported_exp
      msg = 'Expexted reprted should be returned' ).

    cl_abap_unit_assert=>assert_initial(
      act = mo_cut->mt_charc_change_data
      msg = 'No change buffer expected' ).

  ENDMETHOD.