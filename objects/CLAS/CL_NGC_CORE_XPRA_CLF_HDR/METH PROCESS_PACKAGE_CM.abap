METHOD PROCESS_PACKAGE_CM.
  ...
  MESSAGE i034(upgba) WITH 'Received' lv_packcnt 'packages to be processed in phase CURM' INTO cl_upgba_logger=>mv_logmsg ##NO_TEXT.
  ...
  LOOP AT it_packsel ASSIGNING <ls_packsel>.
    ...
    TEST-SEAM combine_seltabs_cm_seam.
      lv_packsel_whr = cl_shdb_pfw_seltab=>combine_seltabs( <ls_packsel>-selection ).
    END-TEST-SEAM.

    TEST-SEAM select_cds_cm_seam.
      SELECT *
        FROM (gc_clfhdrm_name)                           "#EC CI_DYNTAB
