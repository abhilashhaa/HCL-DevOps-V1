METHOD PROCESS_PACKAGE_CS.
  ...
  MESSAGE i034(upgba) WITH 'Received' lv_packcnt 'packages to be processed in phase CURS' INTO cl_upgba_logger=>mv_logmsg ##NO_TEXT.
  ...
  LOOP AT it_packsel ASSIGNING <ls_packsel>.
    ...
    TEST-SEAM combine_seltabs_cs_seam.
      lv_packsel_whr = cl_shdb_pfw_seltab=>combine_seltabs( <ls_packsel>-selection ).
    END-TEST-SEAM.

    TEST-SEAM select_cds_cs_seam.
    SELECT *
      FROM (gc_clfhdrs_name)                           "#EC CI_DYNTAB
    ...
    " Update the table
    UPDATE clf_hdr CLIENT SPECIFIED FROM TABLE @lt_clf_hdr. "#EC CI_CLIENT
