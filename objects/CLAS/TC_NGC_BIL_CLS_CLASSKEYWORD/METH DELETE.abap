...
*--------------------------------------------------------------------*
    ...
    DATA(lt_class_keyword) = VALUE if_ngc_bil_cls_c=>lty_clfnclasskeywordtp-t_delete(
    ...
    DATA(lt_buffer_exp) = VALUE cl_ngc_bil_cls=>lty_t_class_change(
      ( classinternalid  = th_ngc_bil_cls_data=>cs_class_existing-classinternalid
        ...
        t_classkeyword_new = VALUE #(
          ( classkeywordpositionnumber = '02' catchword = th_ngc_bil_cls_data=>cs_classkeyword_existing-classkeywordtext langu = sy-langu delete_flag = abap_true ) )
        t_operation_log  = VALUE #(
          ( cl_ngc_bil_cls=>gc_operation_type-delete_keyword ) ) ) ).
