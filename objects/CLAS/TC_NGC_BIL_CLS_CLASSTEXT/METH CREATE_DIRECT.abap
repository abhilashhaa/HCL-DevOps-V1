...
*--------------------------------------------------------------------*
    ...
    DATA(lt_class_text) = VALUE if_ngc_bil_cls_c=>lty_clfnclasstexttp-t_create_direct(
      ( %cid            = th_ngc_bil_cls_data=>cs_classtext_new-cid
        classinternalid = th_ngc_bil_cls_data=>cs_class_existing-classinternalid
        classtext       = th_ngc_bil_cls_data=>cs_classtext_new-classtext
        longtextid      = th_ngc_bil_cls_data=>cs_classtext_new-longtextid ) ).

    DATA(lt_mapped_exp) = VALUE if_ngc_bil_cls_c=>lty_clfnclasstexttp-t_mapped(
      ( %cid            = th_ngc_bil_cls_data=>cs_classtext_new-cid
        classinternalid = th_ngc_bil_cls_data=>cs_class_existing-classinternalid
        language        = sy-langu
        longtextid      = th_ngc_bil_cls_data=>cs_classtext_new-longtextid  ) ).
    ...
    DATA(lt_buffer_exp) = VALUE cl_ngc_bil_cls=>lty_t_class_change(
      ( classinternalid  = th_ngc_bil_cls_data=>cs_class_existing-classinternalid
        ...
        t_classtext_new  = VALUE #(
          ( langu = th_ngc_bil_cls_data=>cs_classtext_existing-language text_type = th_ngc_bil_cls_data=>cs_classtext_existing-longtextid_00 text_descr = th_ngc_bil_cls_data=>cs_classtext_new-classtext ) )
        t_operation_log  = VALUE #(
          ( cl_ngc_bil_cls=>gc_operation_type-create_text ) ) ) ).
...
*--------------------------------------------------------------------*
    ...
    mo_cut->if_ngc_bil_cls~create_class_text_direct(
      EXPORTING
