METHOD if_ngc_chr_api_read~read.
  ...
  DATA:
    ...
    lt_core_charc_key       TYPE ngct_core_charc_key,
    ls_core_charc_key       LIKE LINE OF lt_core_charc_key,
...
* Check if input key is supplied
  ...
  MOVE-CORRESPONDING it_characteristic_key TO lt_core_charc_key.
  SORT lt_core_charc_key ASCENDING BY key_date charcinternalid.
  DELETE ADJACENT DUPLICATES FROM lt_core_charc_key COMPARING key_date charcinternalid.

* get characteristic data from persistency
  mo_chr_persistency->read_by_internal_key(
    EXPORTING it_key                      = lt_core_charc_key
              iv_lock                     = abap_false
