METHOD SEND_CLSMAS_IDOC.
  ...
  DATA:
    ...
    ls_drf_cls_objkey     TYPE ngcs_drf_clsmas_object_key.
  ...
  mo_drf_ale_info->determine_message_type(
    EXPORTING
      iv_mestyp        = CONV #( if_ngc_drf_c=>gc_idoc_type_clsmas )
...
*     es_message       =
  ...
  LOOP AT it_cls_key ASSIGNING FIELD-SYMBOL(<ls_cls_key>).

    CLEAR: lv_created_comm_idocs.

    mo_idoc_functions->masteridoc_create_clsmas(
      EXPORTING
        klart              = <ls_cls_key>-klart
        klasse             = <ls_cls_key>-class
*        key_date           = SY-DATUM
*        change_number      =
        rcvpfc             = ' '
...
*        mescod             = '   '
    ...
    CLEAR: ls_drf_cls_objkey, lv_object_id.
    ls_drf_cls_objkey-class = <ls_cls_key>-class.
    ls_drf_cls_objkey-klart = <ls_cls_key>-klart.
    " copy to one field
    lv_object_id = ls_drf_cls_objkey.
    update_replication_data(
      EXPORTING
        iv_object_id   = lv_object_id
        it_idoc_result = lt_idoc_control
        iv_messagetype = if_ngc_drf_c=>gc_idoc_type_clsmas
      CHANGING
  ...
  IF ms_runtime_param-bal IS BOUND.
    MESSAGE i000(cmd_bs_mat_drf_msg) WITH if_ngc_drf_c=>gc_idoc_type_clsmas lv_total_idoc INTO DATA(lv_msg) ##NEEDED .
    ...
    LOOP AT ct_idoc_cntrl ASSIGNING FIELD-SYMBOL(<ls_idoc_control>).
      ms_runtime_param-bal->add_msg_idoc(
        ...
        iv_serv_impl   = if_ngc_drf_c=>gc_clsmas_drf_outb_impl " Outbound Implementation for Class IDOC CLSMAS
*       ir_object      =
      ).
    ENDLOOP.
  ENDIF.
