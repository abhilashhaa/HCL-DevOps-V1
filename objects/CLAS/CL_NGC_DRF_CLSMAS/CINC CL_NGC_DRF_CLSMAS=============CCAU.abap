...
CLASS lth_drf_access_cust_data DEFINITION FINAL.
  PUBLIC SECTION.
    CLASS-DATA: gt_cls_key TYPE ngct_drf_clsmas_object_key.
    CLASS-METHODS: class_constructor.
ENDCLASS.

CLASS lth_drf_access_cust_data IMPLEMENTATION.
  METHOD class_constructor.
    gt_cls_key = VALUE #( ( klart = '001' class = 'CLS_1' )
...
CLASS ltd_idoc_functions IMPLEMENTATION.
  METHOD lif_idoc_functions~masteridoc_create_clsmas.
...
CLASS ltc_ngc_clsmas_drf DEFINITION DEFERRED.
CLASS cl_ngc_drf_clsmas DEFINITION LOCAL FRIENDS ltc_ngc_clsmas_drf.

CLASS ltc_ngc_clsmas_drf DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA:
      f_cut TYPE REF TO cl_ngc_drf_clsmas.  "class under test
...
CLASS ltc_ngc_clsmas_drf IMPLEMENTATION.
  ...
  METHOD setup.
    CREATE OBJECT f_cut.
*    CREATE OBJECT f_cut->mo_ngc_drf_utility TYPE ltd_ngc_drf_utility.
...
*      ct_changed_objects TYPE ngct_clfmas_drf_object_key.
*
...
*    ).
  ...
  METHOD initialize.
    ...
    cl_ngc_drf_clsmas=>if_drf_outbound~initialize(
      EXPORTING
    ...
    cl_abap_unit_assert=>assert_equals(
      act = cl_ngc_drf_clsmas=>go_drf_clsmas->ms_runtime_param-appl
      exp = 'TESTAPPL'
      msg = 'initialize: CL_CACL_COM_CLSMAS_DRF=>GO_DRF_CLSMAS->MS_RUNTIME_PARAM-APPL is not valid'
    ).

    cl_abap_unit_assert=>assert_equals(
      act = es_runtime_param_out_impl-table_type_name
      exp = if_ngc_drf_c=>gc_clsmas_drf_table_type
  ...
  METHOD read_complete_data.
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      msg = 'read_complete_data: LT_RELEVANT_OBJECTS is changed'
    ).
