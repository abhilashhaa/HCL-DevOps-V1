METHOD convert_client.
  ...
  IF mv_rework EQ abap_false.
    ...
    SELECT objek FROM ausp CLIENT SPECIFIED
...
*     it is bets pratice to issue a log message with date and time information at the start and end of each logical data conversion phase.
    MESSAGE i035(upgba) WITH 'Starting to update DATUB in AUSP. Client:' mv_client 'Time:' cl_ngc_core_data_conv=>get_current_time( ) INTO cl_upgba_logger=>mv_logmsg.
    ...
    TRY.
      ...
      gr_packprov->add_table( iv_tabname = 'AUSP'
                              ir_conf = gr_tab_conf ).
      ...
      MESSAGE i035(upgba) WITH 'Update finished for DATUB in AUSP. Client:' mv_client 'Time:' cl_ngc_core_data_conv=>get_current_time( ) INTO cl_upgba_logger=>mv_logmsg.
      cl_upgba_logger=>log->trace_single( ).
