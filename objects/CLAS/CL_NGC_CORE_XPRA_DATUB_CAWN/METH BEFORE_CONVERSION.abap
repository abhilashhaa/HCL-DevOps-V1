METHOD before_conversion.
  ...
  IF ( rv_exit EQ abap_false AND iv_rework EQ abap_false ).
    ...
    IF iv_client IS INITIAL.
      SELECT SINGLE 1 INTO @lv_exists FROM cawn CLIENT SPECIFIED WHERE datub = '00000000'.
    ELSE.
      SELECT SINGLE 1 INTO @lv_exists FROM cawn CLIENT SPECIFIED WHERE mandt = @iv_client AND datub = '00000000'.
    ENDIF.
    IF lv_exists NE 1.
