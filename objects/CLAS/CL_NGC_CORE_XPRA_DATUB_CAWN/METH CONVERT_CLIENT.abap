METHOD convert_client.
  ...
  IF mv_rework EQ abap_false.
    ...
    SELECT atinn FROM cawn CLIENT SPECIFIED
...
*   it is bets pratice to issue a log message with date and time information at the start and end of each logical data conversion phase.
    MESSAGE i035(upgba) WITH 'Starting to update DATUB in CAWN. Client:' mv_client 'Time:' cl_ngc_core_data_conv=>get_current_time( ) INTO cl_upgba_logger=>mv_logmsg.
    ...
    TRY.
      ...
      gr_tab_conf->set( iv_key = if_shdb_pfw_package_provider=>mc_conf-upd_tables
                        iv_val = VALUE ltty_updtabs( ( 'CAWN' ) ) ).
      ...
      gr_packprov->add_table( iv_tabname = 'CAWN'
                              ir_conf = gr_tab_conf ).
      ...
      MESSAGE i035(upgba) WITH 'Update finished for DATUB in CAWN. Client:' mv_client 'Time:' cl_ngc_core_data_conv=>get_current_time( ) INTO cl_upgba_logger=>mv_logmsg.
      cl_upgba_logger=>log->trace_single( ).
