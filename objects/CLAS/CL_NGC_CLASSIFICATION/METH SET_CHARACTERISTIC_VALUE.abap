  METHOD set_characteristic_value.
    ...
    LOOP AT lt_valuation_data_in ASSIGNING FIELD-SYMBOL(<ls_valuation_data_in>).
      ...
      IF ls_charc_header-multiplevaluesareallowed = abap_false.
        LOOP AT mt_valuation_data_h TRANSPORTING NO FIELDS
          WHERE
            charcinternalid = <ls_valuation_data_in>-charcinternalid AND
            classtype = <ls_valuation_data_in>-classtype AND
            object_state <> if_ngc_c=>gc_object_state-deleted.
          DATA(lv_value_exists) = abap_true.
        ENDLOOP.

        IF lv_value_exists = abap_true.
          DATA lr_ref TYPE REF TO data.
          FIELD-SYMBOLS <ls_ref> TYPE ngcs_clf_characteristic_key.
          DATA(ls_charc_ref_key) = VALUE ngcs_clf_characteristic_key(
            charcinternalid            = ls_charc_header-charcinternalid
            classtype                  = <ls_valuation_data_in>-classtype
            key_date                   = ls_charc_header-key_date
            overwrittencharcinternalid = ls_charc_header-overwrittencharcinternalid ).
          CREATE DATA lr_ref TYPE ngcs_clf_characteristic_key.
          ASSIGN lr_ref->* TO <ls_ref>.
          <ls_ref> = ls_charc_ref_key.

          MESSAGE e028(ngc_api_base) WITH ls_charc_header-characteristic INTO DATA(lv_message).
...
*                     charcvalueold        = should be initial!
...
*                     charcvalueold        = should be initial!
    ...
    LOOP AT lt_change_value_atwrt_i ASSIGNING FIELD-SYMBOL(<ls_change_value_atwrt_i>).
      ...
      <ls_new_valuation_data>-clfnobjectid = me->ms_classification_key-object_key.
      ...
      APPEND INITIAL LINE TO et_success_value ASSIGNING FIELD-SYMBOL(<ls_success_value>).
      MOVE-CORRESPONDING <ls_new_valuation_data> TO <ls_success_value>.
