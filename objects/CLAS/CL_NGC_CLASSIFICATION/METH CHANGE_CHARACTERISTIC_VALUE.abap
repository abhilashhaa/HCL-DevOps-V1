  METHOD change_characteristic_value.
    ...
    me->if_ngc_classification~get_assigned_values(
      IMPORTING
        et_valuation_data = DATA(lt_valuation_data_old)
    ...
    LOOP AT lt_valuation_data_in ASSIGNING FIELD-SYMBOL(<ls_valuation_data_in>).
      ...
      " These fields are mandatory in the input
      ASSERT <ls_valuation_data_in>-charcvaluepositionnumber IS NOT INITIAL.
      ASSERT <ls_valuation_data_in>-charcinternalid IS NOT INITIAL.
      ASSERT <ls_valuation_data_in>-classtype IS NOT INITIAL.
      ...
      " Look for already assigned values
      READ TABLE lt_valuation_data_old ASSIGNING FIELD-SYMBOL(<ls_valuation_data_old>)
        WITH KEY charcinternalid          = <ls_valuation_data_in>-charcinternalid
                 charcvaluepositionnumber = <ls_valuation_data_in>-charcvaluepositionnumber
                 classtype                = <ls_valuation_data_in>-classtype.
      IF sy-subrc <> 0.
        " Characteristic &1, value position &2 is not assigned
        MESSAGE e010(ngc_api_base) WITH <ls_valuation_data_in>-charcinternalid <ls_valuation_data_in>-charcvaluepositionnumber INTO lv_msg ##NEEDED.
        lo_clf_api_result->add_message_from_sy( me->ms_classification_key ).
        CONTINUE.
      ENDIF.
      ...
      TRY.
          ...
          cl_ngc_util_message=>convert_msg_corechr_to_clf(
            EXPORTING
              is_classification_key       = me->ms_classification_key
              is_clf_characteristic_key   = VALUE #( classtype       = <ls_valuation_data_in>-classtype
                                                     charcinternalid = ls_core_charc_header-charcinternalid
                                                     key_date        = ls_core_charc_header-key_date )
              iv_charcvaluepositionnumber = <ls_valuation_data_in>-charcvaluepositionnumber " TODO
              it_core_charc_msg           = lt_core_chr_message
            IMPORTING
              et_classification_msg       = DATA(lt_clf_message)
          ).
          lo_clf_api_result->add_messages( lt_clf_message ).
    ...
    LOOP AT lt_valuation_data ASSIGNING FIELD-SYMBOL(<ls_valuation_data>).
      ...
      " Look for already assigned values
      READ TABLE lt_valuation_data_old ASSIGNING <ls_valuation_data_old>
        WITH KEY charcinternalid          = <ls_valuation_data_in>-charcinternalid
                 charcvaluepositionnumber = <ls_valuation_data_in>-charcvaluepositionnumber
                 classtype                = <ls_valuation_data_in>-classtype.
      IF sy-subrc <> 0.
        " there was already a check for this earlier, so here we just skip it
        CONTINUE.
      ENDIF.

      APPEND VALUE #( classtype            = <ls_valuation_data>-classtype
                      charcinternalid      = <ls_valuation_data>-charcinternalid
                      charcvalueold        = <ls_valuation_data_old>-charcvalue " here we use the old value
                      charcvaluenew        = <ls_valuation_data>-charcvalue
                      valuation_data_index = lv_index ) TO lt_change_value_atwrt_i.
      APPEND VALUE #( classtype            = <ls_valuation_data>-classtype
                      charcinternalid      = <ls_valuation_data>-charcinternalid
                      charcvalueold        = <ls_valuation_data_old>-charcvalue " here we use the old value
                      charcvaluenew        = <ls_valuation_data>-charcvalue ) TO lt_change_value_atwrt.
    ENDLOOP.
