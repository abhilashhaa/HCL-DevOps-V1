METHOD PROCESS_PACKAGE.
  ...
  DATA:
    ...
    lt_ksml_update   TYPE STANDARD TABLE OF ksml,
  ...
  LOOP AT it_packsel ASSIGNING <ls_packsel>.
    ...
    SELECT * FROM ksml CLIENT SPECIFIED
      WHERE (lv_packsel_whr)
      ORDER BY clint, posnr, datuv DESCENDING
      INTO TABLE @lt_ksml.
    LOOP AT lt_ksml ASSIGNING FIELD-SYMBOL(<ls_ksml>).
      ...
      IF <ls_ksml>-clint NE lv_clint
      OR <ls_ksml>-posnr NE lv_posnr.
        lv_valid_to = lc_max_date.
        lv_clint    = <ls_ksml>-clint.
        lv_posnr    = <ls_ksml>-posnr.
      ENDIF.
      ...
      IF <ls_ksml>-datub NE lv_valid_to.
        <ls_ksml>-datub = lv_valid_to.
        APPEND <ls_ksml> TO lt_ksml_update.
      ENDIF.
      ...
      lv_valid_to = <ls_ksml>-datuv - 1.
    ENDLOOP.

    UPDATE ksml CLIENT SPECIFIED FROM TABLE @lt_ksml_update.
