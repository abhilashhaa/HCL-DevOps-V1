...
CLASS ltc_ngc_clf_val_ecn_used DEFINITION DEFERRED.
CLASS cl_ngc_clf_val_ecn_used DEFINITION LOCAL FRIENDS ltc_ngc_clf_val_ecn_used.

CLASS ltc_ngc_clf_val_ecn_used DEFINITION FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    DATA:
      mo_cut TYPE REF TO cl_ngc_clf_val_ecn_used.
...
CLASS ltc_ngc_clf_val_ecn_used IMPLEMENTATION.
...
*    DATA(lo_clf_api_result) = mo_cut->if_ngc_clf_validator~validate( iv_classtype      = lth_ngc_clf_val_ecn_classtype=>gc_classtype_without_ecn_supp
*                                                                     io_data_provider  = NEW ltd_ngc_clf_validation_dp( )
*                                                                     io_classification = NEW ltd_ngc_clf_classifictn_ecn( iv_classtype_ecn_support = abap_false
*                                                                                                                          iv_has_assigned_classes  = abap_true
*                                                                                                                          iv_ecn_clf_support       = abap_true ) ).
*
*    cl_abap_unit_assert=>assert_bound( act = lo_clf_api_result
...
*    DATA(lo_clf_api_result) = mo_cut->if_ngc_clf_validator~validate( iv_classtype      = lth_ngc_clf_val_ecn_classtype=>gc_classtype_with_ecn_supp
*                                                                     io_data_provider  = NEW ltd_ngc_clf_validation_dp( )
*                                                                     io_classification = NEW ltd_ngc_clf_classifictn_ecn( iv_classtype_ecn_support = abap_true
...
*    MESSAGE e008(ngc_api_base) WITH iv_classtype INTO DATA(lv_msg).
