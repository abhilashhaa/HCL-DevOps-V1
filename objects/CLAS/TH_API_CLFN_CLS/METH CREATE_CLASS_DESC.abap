  METHOD create_class_desc.
    DATA(lv_batch_request_class) = th_mmim_odata_request_builder=>start_request(
        )->open_batch_request(
          )->open_changeset(
            )->open_change(
              )->set_http_operation( 'POST'
              )->set_uri( iv_uri = 'A_ClfnClassForKeyDate(ClassInternalID=' && '''' && is_class-classinternalid && '''' && ')' && '/to_ClassDescription'
              )->set_body( iv_body = me->get_body(
                   it_classdesc  = it_classdesc
                   it_fieldname  = VALUE #( ( 'Language' ) ( 'ClassDescription' ) )
                 )
