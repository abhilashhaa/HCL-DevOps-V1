METHOD convert_client.
  ...
  TYPES:
    ...
    lrty_atinn         TYPE RANGE OF atinn,
  ...
  DATA:
    lt_atinn_first     TYPE TABLE OF atinn.

  IF mv_rework EQ abap_false.
    " Determine the first DATUB=00000000 to avoid reprocessing the previously processed rows
    SELECT atinn FROM cawnt CLIENT SPECIFIED
      INTO TABLE @lt_atinn_first
      UP TO 1 ROWS
      WHERE mandt = @mv_client AND datub = '00000000'
      ORDER BY atinn.
  ENDIF.

  IF mv_rework EQ abap_true OR lines( lt_atinn_first ) GT 0.
    ...
    MESSAGE i035(upgba) WITH 'Starting to update DATUB in table CAWNT. Client:' mv_client 'Time:' cl_ngc_core_data_conv=>get_current_time( ) INTO cl_upgba_logger=>mv_logmsg.
    ...
    TRY.
      ...
      gr_tab_conf->set( iv_key = if_shdb_pfw_package_provider=>mc_conf-upd_tables
                        iv_val = VALUE ltty_updtabs( ( 'CAWNT' ) ) ).
      ...
      ELSE.
        DATA(lt_sel_atinn) = VALUE lrty_atinn( ( sign = 'I' option = 'GE' low = lt_atinn_first[ 1 ] high = lt_atinn_first[ 1 ] ) ).
        gr_tab_conf->set( iv_key = if_shdb_pfw_package_provider=>mc_conf-filter
                          iv_val = VALUE ltty_named_seltab(
                            ( name = 'MANDT' dref = REF #( lt_sel_mandt ) )
                            ( name = 'ATINN' dref = REF #( lt_sel_atinn ) )
                          )
                        ).
      ENDIF.
      "create the package provider
      DATA(gr_packprov) = cl_shdb_pfw_factory=>new_package_provider( ).
      gr_packprov->add_table( iv_tabname = 'CAWNT'
                              ir_conf = gr_tab_conf ).
      ...
      MESSAGE i035(upgba) WITH 'Update finished for DATUB in CAWNT. Client:' mv_client 'Time:' cl_ngc_core_data_conv=>get_current_time( ) INTO cl_upgba_logger=>mv_logmsg.
