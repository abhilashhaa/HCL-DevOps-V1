CLASS lth_test_data DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-DATA:
      gs_charcval_authgrp_empty  TYPE i_clfncharcvalue,
      gs_charcval_authgrp_auth   TYPE i_clfncharcvalue,
      gs_charcval_authgrp_noauth TYPE i_clfncharcvalue,
...
CLASS lth_test_data IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    gs_charcval_authgrp_empty = VALUE #(
      charcinternalid          = '1'
    ...
    gs_charcval_authgrp_auth = VALUE #(
      charcinternalid          = '2'
      charcvaluepositionnumber = '1'
      validitystartdate        = if_ngc_c=>gc_date_min
      validityenddate          = if_ngc_c=>gc_date_max
    ...
    gs_charcval_authgrp_noauth = VALUE #(
      charcinternalid          = '3'
      charcvaluepositionnumber = '1'
      validitystartdate        = if_ngc_c=>gc_date_min
      validityenddate          = if_ngc_c=>gc_date_max
...
"! @testing I_ClfnCharcValueForKeyDate
CLASS ltc_i_clfncharcforkd_auth DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ltt_i_clfncharcvalueforkd TYPE STANDARD TABLE OF i_clfncharcvalueforkeydate WITH EMPTY KEY,
      ltt_i_clfncharcvalue      TYPE STANDARD TABLE OF i_clfncharcvalue WITH EMPTY KEY,
      ltt_i_clfncharcforkd      TYPE STANDARD TABLE OF i_clfncharacteristicforkeydate WITH EMPTY KEY.
    ...
    METHODS:
      read_cds
        RETURNING
          VALUE(rt_result) TYPE ltt_i_clfncharcvalueforkd,
...
CLASS ltc_i_clfncharcforkd_auth IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    go_environment = cl_cds_test_environment=>create(
      i_for_entity      = 'I_ClfnCharcValueForKeyDate'
  ...
  METHOD setup.
    ...
    DATA(lt_charcvalue) = VALUE ltt_i_clfncharcvalue(
      ( lth_test_data=>gs_charcval_authgrp_empty )
      ( lth_test_data=>gs_charcval_authgrp_auth )
      ( lth_test_data=>gs_charcval_authgrp_noauth )
    ).
    go_environment->insert_test_data( lt_charcvalue ).

    DATA(lt_charc) = VALUE ltt_i_clfncharcforkd(
  ...
  METHOD read_cds.
    ...
    SELECT * FROM i_clfncharcvalueforkeydate
